...
public class DownloadNSEHistoricalData {
    ...
    public static void downloadHistoricalData(SmartConnect smartConnect) {
        ...
        for (int i = 0; i < tokensList.length; i++) {
            ...
            LocalDate startDate = LocalDate.of(2024, 01, 01); // Start date of historical data

            while (startDate.isBefore(endDate)) {
                LocalDate nextMonth = startDate.plus(1, ChronoUnit.MONTHS);
                ...
                requestObject.put("todate", nextMonth.format(formatter) + " 15:30");

                logger.info("Fetching data for " + tokensName[i] + " from " + startDate.format(formatter) + " to " + nextMonth.format(formatter));
